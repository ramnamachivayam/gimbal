...
def populate_circle():
    for i in range(1, 2):
        ...
        attributes = {
            GLOBAL_ATTRIBUTE_KEY + 'circle-' + str(i): GLOBAL_ATTRIBUTE_NAME + 'circle-' + str(i)
        }
        data = {
            'name': GLOBAL_NAME_CIRCLE + str(i),
            'geofence': geo,
            'beacons': [],
...
def populate_no_geo_beacons():
    for i in range(38434, 50000):
        attributes = {
            GLOBAL_ATTRIBUTE_KEY + 'no-geo-beacon-' + str(i): GLOBAL_ATTRIBUTE_NAME + 'no-geo-beacon-' + str(i)
        }

        beacons = get_beacons_list()

        data = {
            'name': GLOBAL_NAME_NO_GEO_BEACON + str(i),
            'geofence': None,
            'beacons': beacons,
            'attributes': attributes
        }
